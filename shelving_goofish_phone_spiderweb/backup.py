...
def sync_folder_to_phone(device, local_folder, remote_dir, label="图片"):
    ...
    # --- 2. 异步监听器 ---
    def monitor_progress(expected_count):
        print("🔍 启动图片推送监听器...")
        uploaded_count = 0
        while uploaded_count < expected_count:
            # 统计手机端已存在的图片数量
            res = d.shell(f"ls {remote_dir} | grep IMG_ | wc -l")
    ...
    monitor_thread = threading.Thread(target=monitor_progress, args=(total_count,))
...
# --- 自动化填写流程 ---
def start_shelving_process(goods_id):
    # 1. 准备数据
    ...
    spec_remote = "/sdcard/Pictures/Gallery/owner/商品规格图/"
    ...
    # 这里可以调用你之前的 title_generation 生成标题填入，或直接填入描述
    ...
    print("✅ 填写完成，请自行确认并完成发布。")


# 执行
if __name__ == "__main__":
    target_id = "751452696227"
    ip_port = "172.20.10.4:42367"

    d = u2.connect(ip_port)
    start_shelving_process(target_id)