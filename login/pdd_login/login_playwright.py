...
def wait_and_fill_code(page, timeout=60):
    """
    每秒去 FastAPI 后端检查一次，直到拿到验证码并填入
    """
    print("⏳ 正在等待后端转发验证码...")
    ...
    while time.time() - start_time < timeout:
        try:
            # 请求你自己写的 FastAPI 接口
            response = requests.get("http://127.0.0.1:8080/get_code").json()
            code = response.get("code")

            if code:
                print(f"🚀 成功获取实时码 {code}，正在填入...")
...
def run():
    ...
    with sync_playwright() as p:
        ...
        # 1. 访问淘宝登录页
        print("正在跳转至拼多多登录页...")
        page.goto("https://mobile.pinduoduo.com/?page_id=10002_1773482380204_1ivqf1rd5u&is_back=1&bsch_is_search_mall=&bsch_show_active_page=")
        time.sleep(1)

        page.get_by_text("个人中心").click()
        page.get_by_text("手机登录").click()
        page.fill("#user-mobile", MY_PHONE)

        page.click("#code-button")
        wait_and_fill_code(page)

        page.locator("i.agreement-icon").click()
        ...
        try:
            ...
            with open("../cookie/pdd_cookie/pdd_cookies.json", "w", encoding="utf-8") as f:
                json.dump(cookies, f, ensure_ascii=False, indent=4)
            print("✅ Cookie 已成功保存至 pdd_cookies.json")
