...
SHELF_SAVE_DIR = r"C:\Users\31193\Pictures\上架图"
...
async def main():
    # 1. 用户交互输入
    target_url = input("请输入拼多多商品URL: ").strip()

    # 2. 自动化参数解析
    goods_id = get_goods_id(target_url)
    if not goods_id:
    ...
    print(f"\n🚀 任务启动 | 商品ID: {goods_id}")
    print(f"📂 详情图目录: {save_path}")
    print(f"📂 实拍图目录: {review_path}")

    # --- 步骤 1: 下载商品详情图 ---
    print("\n--- [Step 1/5] 正在下载商品详情图 ---")
    downloader = PddGoodsDetailDownloader(BASE_SAVE_DIR, goods_id)
    await downloader.run(target_url)
